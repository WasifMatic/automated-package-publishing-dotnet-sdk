...
using PaypalServer.Standard.Exceptions;
using PaypalServer.Standard.Http.Response;
using System.Net.Http;
using System.Threading;
using System.Threading.Tasks;

namespace PaypalServer.Standard.Controllers
{
    /// <summary>
        ...
        public async Task<ApiResponse<Models.BillingPlan>> CreateBillingPlanAsync(
            ...
            => await CreateApiCall<Models.BillingPlan>()
              ...
              .ResponseHandler(responseHandler => responseHandler
                  .ErrorCase("400", CreateErrorCase("Bad Request. Request is not well-formed, syntactically incorrect, or violates schema.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
                  .ErrorCase("401", CreateErrorCase("Authentication failed due to missing authorization header, or invalid authentication credentials.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
                  .ErrorCase("403", CreateErrorCase("Authorization failed due to insufficient permissions.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
                  .ErrorCase("422", CreateErrorCase("The requested action could not be performed, semantically incorrect, or failed business validation.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
                  .ErrorCase("500", CreateErrorCase("An internal server error has occurred.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
                  .ErrorCase("0", CreateErrorCase("The error response.", (errorReason, context) => new SubscriptionErrorException(errorReason, context))))
        ...
        public async Task<ApiResponse<Models.PlanCollection>> ListBillingPlansAsync(
            ...
            => await CreateApiCall<Models.PlanCollection>()
              .RequestBuilder(requestBuilder => requestBuilder
                  ...
                  .Parameters(parameters => parameters
                      .Header(header => header.Setup("Prefer", input.Prefer))
                      .Query(query => query.Setup("product_id", input.ProductId))
              ...
              .ResponseHandler(responseHandler => responseHandler
                  .ErrorCase("400", CreateErrorCase("Request is not well-formed, syntactically incorrect, or violates schema.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
                  .ErrorCase("401", CreateErrorCase("Authentication failed due to missing authorization header, or invalid authentication credentials.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
                  .ErrorCase("403", CreateErrorCase("Authorization failed due to insufficient permissions.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
                  .ErrorCase("404", CreateErrorCase("The specified resource does not exist.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
                  .ErrorCase("500", CreateErrorCase("An internal server error has occurred.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
                  .ErrorCase("0", CreateErrorCase("The error response.", (errorReason, context) => new SubscriptionErrorException(errorReason, context))))
              .ExecuteAsync(cancellationToken).ConfigureAwait(false);
        ...
        public async Task<ApiResponse<Models.SubscriptionCollection>> ListSubscriptionsAsync(
            ...
            => await CreateApiCall<Models.SubscriptionCollection>()
              .RequestBuilder(requestBuilder => requestBuilder
                  ...
                  .WithAuth("Oauth2")
                  .Parameters(parameters => parameters
                      .Query(query => query.Setup("plan_ids", input.PlanIds))
                      .Query(query => query.Setup("statuses", input.Statuses))
                      .Query(query => query.Setup("created_after", input.CreatedAfter))
                      .Query(query => query.Setup("created_before", input.CreatedBefore))
                      .Query(query => query.Setup("status_updated_before", input.StatusUpdatedBefore))
                      .Query(query => query.Setup("status_updated_after", input.StatusUpdatedAfter))
        ...
        public async Task<ApiResponse<Models.TransactionsList>> ListSubscriptionTransactionsAsync(
            ...
            => await CreateApiCall<Models.TransactionsList>()
              .RequestBuilder(requestBuilder => requestBuilder
                  ...
                  .Parameters(parameters => parameters
                      .Template(template => template.Setup("id", input.Id))
                      .Query(query => query.Setup("start_time", input.StartTime))
                      .Query(query => query.Setup("end_time", input.EndTime))))
              .ResponseHandler(responseHandler => responseHandler
                  .ErrorCase("400", CreateErrorCase("Bad Request. Request is not well-formed, syntactically incorrect, or violates schema.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
                  .ErrorCase("401", CreateErrorCase("Authentication failed due to missing authorization header, or invalid authentication credentials.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
                  .ErrorCase("403", CreateErrorCase("Authorization failed due to insufficient permissions.", (errorReason, context) => new SubscriptionErrorException(errorReason, context)))
