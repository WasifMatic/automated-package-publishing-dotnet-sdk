...
Input structure for the method SearchTransactions
...
| `TransactionType` | `string` | Optional | Filters the transactions in the response by a PayPal transaction event code. See [Transaction event codes](/docs/integration/direct/transaction-search/transaction-event-codes/). |
...
| `TransactionCurrency` | `string` | Optional | Filters the transactions in the response by a [three-character ISO-4217 currency code](https://developer.paypal.com/api/rest/reference/currency-codes/) for the PayPal transaction currency. |
...
| `StoreId` | `string` | Optional | Filters the transactions in the response by a store ID. |
| `TerminalId` | `string` | Optional | Filters the transactions in the response by a terminal ID. |
...
| `BalanceAffectingRecordsOnly` | `string` | Optional | Indicates whether the response includes only balance-impacting transactions or all transactions. Value is either: Y. The default. The response includes only balance transactions. N. The response includes all transactions.<br><br>**Default**: `"Y"` |
| `PageSize` | `int?` | Optional | The number of items to return in the response. So, the combination of `page=1` and `page_size=20` returns the first 20 items. The combination of `page=2` and `page_size=20` returns the next 20 items.<br><br>**Default**: `100`<br><br>**Constraints**: `>= 1`, `<= 500` |
| `Page` | `int?` | Optional | The zero-relative start index of the entire list of items that are returned in the response. So, the combination of `page=1` and `page_size=20` returns the first 20 items.<br><br>**Default**: `1`<br><br>**Constraints**: `>= 1`, `<= 2147483647` |
...
SearchTransactionsInput searchTransactionsInput = new SearchTransactionsInput
{
    ...
    TransactionStatus = "transaction_status0",
    TransactionAmount = "transaction_amount0",
    TransactionCurrency = "transaction_currency8",
    PaymentInstrumentType = "payment_instrument_type2",
    StoreId = "store_id2",
    TerminalId = "terminal_id4",
